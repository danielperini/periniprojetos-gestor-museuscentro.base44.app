...
function normalizeMuseu(value) {
  ...
  if (text === 'mhab' || text === 'mab' || text.includes('abilio') || text.includes('historico')) return 'MHAB';
...
function isNoturno(r = {}) {
  return normalizeText(r?.escopo_orcamentario || r?.escopo || '').includes('noturno') || getSearchText(r, true).includes('noturno');
}
...
function matchRubricaMuseu(r = {}, museu = '') {
  ...
  const rubricaMuseu = normalizeMuseu(r?.museu_codigo || r?.museu || r?.centro_custo || '');

  if (!normalizedMuseu || normalizedMuseu === 'GERAL') return false;
  if (r?.excluir_cards_museu === true) return false;
  if (isHiddenRubrica(explicitText)) return false;
  if (normalizedMuseu === 'NOTURNO') return isNoturno(r);
  if (isNoturno(r)) return false;
  if (rubricaMuseu) return rubricaMuseu === normalizedMuseu;
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-6 md:py-10 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <KpiCard label="Utilizado" value={fmt(totaisGerais.totalUtilizado)} helper="rubricas específicas" />
        ...
        <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
          <div className="px-4 py-4 border-b border-gray-100 flex items-center justify-between gap-3 flex-wrap">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">Visualização e edição das rubricas operacionais específicas por museu e Noturno.</p>
            </div>
