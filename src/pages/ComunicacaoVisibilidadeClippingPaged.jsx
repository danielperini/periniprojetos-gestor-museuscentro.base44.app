...
export default function ComunicacaoVisibilidadeClippingPaged() {
  ...
  function setPage(monthKey, page, totalPages) {
    const nextPage = Math.max(0, Math.min(page, Math.max(0, totalPages - 1)));
    setMonthPages((prev) => ({ ...prev, [monthKey]: nextPage }));
  }
          ...
          {clippingByMonth.length === 0 ? (
            ...
            <div className="space-y-5">
              {clippingByMonth.map((group) => {
                ...
                const page = Math.min(getPage(group.key), Math.max(0, totalPages - 1));
                const visibleItems = group.items.slice(page * PAGE_SIZE, page * PAGE_SIZE + PAGE_SIZE);
                ...
                return (
                  <section key={group.key} className="space-y-2">
                    ...
                    <MonthPager page={page} totalPages={totalPages} onPage={(nextPage) => setPage(group.key, nextPage, totalPages)} />
                  </section>
                );
              })}
            </div>
