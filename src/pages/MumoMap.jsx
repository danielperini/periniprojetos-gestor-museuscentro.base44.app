...
export default function MumoMap() {
  const [isAnalyzing, setIsAnalyzing] = useState(false);

  const { data: pontos = [], refetch } = useQuery({
    queryKey: ['pontos-mumo'],
    queryFn: () => base44.entities.PontoEntorno.filter({ museu_sigla: 'MUMO', ativo: true }),
  });
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-6 md:py-10">
            ...
            <div className="flex items-start gap-3 mb-4">
              ...
              <div className="flex-1">
                ...
                <div className="space-y-2 text-sm text-gray-700">
                  <p>
                    <strong>Total de Pontos Identificados:</strong> {analise.total} instituições/locais mapeados para mobilização do MUMO
                  </p>
                  <p>
                    <strong>Prioridade Alta:</strong> {analise.altaPrioridade} {analise.altaPrioridade === 1 ? 'ponto' : 'pontos'} com potencial de ação
                  </p>
                  <p>
                    <strong>Aderência Temática Média:</strong> {analise.aderenciaMedia}% de alinhamento com moda, design e arte contemporânea
                  </p>
        ...
        <SimplifiedMapViewer pontos={pontos} museu="MUMO" />
      </div>
    </div>
  );
}