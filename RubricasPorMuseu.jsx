...
function getBudgetLineLabel(b) {
  ...
  const descricao = String(b.rubrica || b.nome || b.descricao || b.name || '').trim();
  if (codigo && descricao) return `${codigo} — ${descricao}`;
  return codigo || descricao || 'Rubrica';
}

function isTeamRubrica(r) {
  const texto = String([r?.grupo, r?.rubrica, r?.nome, r?.meta].filter(Boolean).join(' '))
    .normalize('NFD')
    .replace(/[̀-ͯ]/g, '')
    .toLowerCase();
  return r?.excluir_cards_museu === true ||
    texto.includes('equipe principal') ||
    texto.includes('coordenador') ||
    texto.includes('assistente') ||
    texto.includes('analista adm') ||
    texto.includes('educador') ||
    texto.includes('producao') ||
    texto.includes('consultoria de programacao');
}
...
export default function TeamMemberForm({ isOpen, onClose, onSuccess, editingMember = null, budgetLines = [] }) {
  ...
  const { data: budgetLinesFromDB = [], isLoading: loadingBudgetLines } = useQuery({
    ...
    queryFn: async () => normalizeListResponse(await base44.entities.Rubrica.list('ordem_exibicao', 5000)),
  ...
  const finalBudgetLines = useMemo(() => {
    ...
    return source
      ...
      .filter(isTeamRubrica)
      .sort((a, b) => String(a.rubrica || a.nome || '').localeCompare(String(b.rubrica || b.nome || ''), 'pt-BR'));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        rubrica_id: normalizeBudgetLineId(form.budgetline_id),
